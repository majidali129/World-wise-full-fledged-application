...
export async function getBookings ({filter, sortBy, page}) {
  ...
  if(filter) query = query[typeof filter.value === 'string'? 'eq': filter.method](filter.field, filter.value);

  // SORT OPERATION
  if(sortBy) query = query.order(sortBy.field, {ascending: sortBy.direction === 'asc'})

  // Pagination Operation
  // this'll work according to array indexing , as it starts from 0 .
  if(page){
